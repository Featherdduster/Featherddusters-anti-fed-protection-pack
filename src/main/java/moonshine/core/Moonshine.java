...
import moonshine.core.registry.ItemRegistry;
import moonshine.core.registry.SoundRegistry;
...
public class Moonshine {
    ...
    public static final ItemGroup GROUP = new ItemGroup(ID) {
        ...
        public ItemStack createIcon() {
            ...
            ItemStack stack = new ItemStack(ItemRegistry.Raiphala_Skh_3.get());
            //Here we add ammunition to the gun so it doesn't have the re-fill bar under the item
            stack.getOrCreateTag().putInt("AmmoCount", ItemRegistry.Raiphala_Skh_3.get().getGun().getReloads().getMaxAmmo());
            ...
            if (TimelessGunItem.class.isAssignableFrom(object.get().getClass())) {
                try {
                    ModelOverrides.register(
                            (Item) object.get(),
                            (IOverrideModel) Class.forName("moonshine.client.render.gun.model." + field.getName().toLowerCase(Locale.ENGLISH) + "_animation").newInstance()
                    );
